...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        assert default_dataset.context_length == 8
        assert default_dataset.token_count == len(corpus)
        ...
        assert default_dataset.raw_text == corpus
        assert len(default_dataset.token_ids) == len(corpus)
        ...
        assert decoded_input == corpus[:8]
        assert decoded_target == corpus[1:9]
