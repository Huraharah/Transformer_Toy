...
# Ensure filestructure exists and CUDA is available

FULL_MODEL_DIRECTORY = Path("./full_model")
TOKENIZER_PATH = FULL_MODEL_DIRECTORY / "tokenizer.tok"
...
# Build configurations and import data
...
# Import data
dataset = tt.TextDataset(file_path = str(CORPUS_PATH), context_length = 128, tokenizer_config = token_config)
...
training_build = build_sequential_batches(token_ids = all_token_ids, range_begin = 0, range_end = split_index, context_length = 128, batch_size = 32)
validation_build = build_sequential_batches(token_ids = all_token_ids, range_begin = split_index, range_end = len(all_token_ids), context_length = 128, batch_size = 32)
...
trans_block_config.d_model = 128
trans_block_config.d_ff = 512
trans_block_config.attention_type = tt.AttentionType.MULTI_HEAD
trans_block_config.num_heads = 4
...
trans_model_config.max_seq_len = 128
trans_model_config.num_layers = 4
...
train_config.generation_every_epochs = 5
...
print("\n" + "=" * 60)
print("Full Model Run Configuration")
print("=" * 60)
...
plt.title("Transformer_Toy Full Model Losses")
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig("./full_model/losses.png")
...
history_path = Path("./full_model/history.csv")
